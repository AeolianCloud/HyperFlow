...
PveClient SHALL 使用 API Token 方式对 PVE REST API 进行认证，认证头格式为 `Authorization: PVEAPIToken=<tokenid>=<secret>`。Token 信息 SHALL 从环境变量 `PVE_TOKEN_ID` 和 `PVE_TOKEN_SECRET` 读取。
